    ...
    <div className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      ...
      >
        ...
        <div className='pt-6'>
          ...
          {
            userState === 'Login'
            && <p className='text-base text-[#007AFF] pt-3 pb-2'>Forget Password?</p>
          }

          <button className='text-white bg-[#007AFF] rounded-full px-8 py-3 w-full'>{userState}</button>
          {
            userState === 'Sign Up'
              ?
              <p className='text-[#7A7A7A] text-base pt-5'>Don’t have an account? <span onClick={() => setUserState('Login')} className='text-[#007AFF]'>Login</span></p>
              :
              <p className='text-[#7A7A7A] text-base pt-5'>Already have a Account? <span onClick={() => setUserState('Sign Up')} className='text-[#007AFF]'>Sign Up</span></p>
          }
